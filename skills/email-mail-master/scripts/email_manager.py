...
def get_email_manager(email_type: str, config: Dict) -> EmailManager:
    ...
    if email_type == 'qq':
        ...
        email_config = config['ali_email']
        return AliEmailManager(
            email_address=email_config['email'],
            auth_password=email_config['auth_password']
        )
    else:
        raise ValueError(f"不支持的邮箱类型: {email_type}，请使用 'qq'、'163' 或 'ali'")


class AliEmailManager(EmailManager):
    """阿里企业邮箱管理器 - 使用 IMAP"""
    ...
    def __init__(self, email_address: str, auth_password: str):
        super().__init__(
            email_address=email_address,
            password=auth_password,
            imap_server='imap.mxmail.creator.cn',
            imap_port=993,
            smtp_server='smtp.mxmail.creator.cn',
            smtp_port=465
        )
